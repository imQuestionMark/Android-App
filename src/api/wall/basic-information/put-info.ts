...
export const basicInfoInputSchema = z.object({
  ...
  currentLocation: z.string({ required_error: 'Select your current location' }),
  about: z
    .string()
...
const basicInfoRequest = async ({
  ...
  const response = await client.post(
    `${API_ROUTES.BASIC_INFORMATION}/${id}`,
    data
  );
  return basicInfoResponseSchema.parse(response.data);
};

export const useUpdateBasicInfoMutation = createMutation<
  Response,
  { data: Variables; id: string },
  Error
>({
